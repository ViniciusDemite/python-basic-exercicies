...
def printResult(l, message):
  print(f'Lista: {l}', message, sep='\n', end='\n\n')

  return None

for l in lista_de_listas_de_inteiros:
  duplicated = findDuplicate(l)

  if duplicated == -1:
    printResult(l, 'Não há nenhum número duplicado')
    continue

  printResult(l, f'A primeira ocorrência de duplicação é o número: {duplicated}')